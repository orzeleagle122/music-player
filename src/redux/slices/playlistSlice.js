...
export const playlistSlice = createSlice({
    ...
    reducers: {
        ...
        deleteSongFromPlaylist: (state, action) => {
            const playlist = state.playlist.find(item => item.id === action.payload.idPlaylist);
            if (playlist) {
                // playlist.songs.filter(item => item.id !== action.payload.idSong);
                // console.log('jestes tu?')
                // return {...state, playlist: {...state.playlist, song: [...r]}}
            }
        }
    }
})
...
export const {searchSongs, moreSongs, addSongToPlaylist, deleteSongFromPlaylist} = playlistSlice.actions;
...
export const deleteSongFromPlaylistAction = (idSong, idPlaylist) => async dispatch => {
    console.log(initialState.playlist.find(item => item.id === idPlaylist))
    dispatch(deleteSongFromPlaylist({idSong, idPlaylist}));
}
